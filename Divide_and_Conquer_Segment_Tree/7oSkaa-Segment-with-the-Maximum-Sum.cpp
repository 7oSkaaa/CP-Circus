...
void Solve(){
    ...
    struct Node {
        ll mx_pref, mx_suff, mx_sum, sum;
        Node(ll x = 0) : mx_pref(x), mx_suff(x), mx_sum(x), sum(x) {}
    };
    auto op = [](const Node& a, const Node& b) -> Node {
        ...
        res.mx_sum = max({a.mx_sum, b.mx_sum, a.mx_suff + b.mx_pref});
        return res;
    };

    Segment_Tree < Node, int > st(n, a, op, Node(0));

    cout << max(0ll, st.query(1, n).mx_sum) << "\n";
    while(q--){
        int idx, val;
        cin >> idx >> val;
        st.update(++idx, val);
